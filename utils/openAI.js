...
export const apiCall = async (prompt, messages)=>{
    ...
    //         messages: [{
    //             role: 'user',
    //             content: `В следующем сообщении указана просьба сгенерировать или показать изображение, предмет искусства или что-то на это похожее? ${prompt} . Ответь только 'yes', если да, и 'no', если нет.`
    //         }]
    //     });
    ...
    let isArt = prompt.includes('представь') || prompt.includes('покажи') || prompt.includes('как выглядит') || prompt.includes('нарисуй') || prompt.includes('drawing');
    if(isArt){
        console.log('dalle api call');
        return dalleApiCall(prompt, messages)
    }else{
